...
- **상태**: 진행 중 — local DOD-01~07·10 통과, clean checkout·public E4·제출 패키지 대기
...
- [ ] **DOD-08 — 재현 빌드**: `npm ci`, `npm run verify`, prototype verifier/smoke, `npm run build:pages`, `npm run test:pages`가 clean checkout에서 exit 0이다. 증거 수준 E3.
- [ ] **DOD-09 — 공개 E4**: 고정 SHA artifact가 GitHub Pages에 배포되고 새 desktop/mobile profile에서 root·stage/signal/seed·직접 prototype·M01 `/#/` bridge가 오류 없이 동작한다. (INV-014, INV-020)
- [x] **DOD-10 — 결함**: 활성 P0/P1·불변식 위반 0건이며 P2 유예는 DOD_GUIDE 절차를 따른다. (INV-018, INV-020)
- [ ] **DOD-11 — 릴리스 정직성**: tag·SHA·artifact hash·URL·README·영상의 기능 주장이 일치하고 Prototype/`v0.1`로 표기된다.
- [ ] **DOD-12 — 제출 패키지**: 동일 SHA 실제 화면의 16:9 썸네일, 3분 이하 영상, Codex/사람 역할, 자산 권리, 2회 필드 대조, 제출 캡처와 2중 백업을 고정한다.
...
| 실제 Pages 새 프로필 | root→stage/signal/seed→M01 `/#/` | 404·console·외부 요청 0 | commit/push 뒤 public E4 대기 |
...
build modules=41 pagesArtifact files=14 bytes=336407 prototypeFiles=10 steps=10
...
pending: committed clean checkout npm ci, fixed-SHA public E4, tag/license/submission package
```
...
- LICENSE와 최종 제출 양식·마감 시각은 H00 완료 전 사람 결정이 필요하다.
...
H00은 정규 roadmap을 진전시키지 않는다. 제출 후 M02는 원래 DoR에서 시작하고 H00 프로토타입은 기준 fixture·UX 관찰로만 참고한다.