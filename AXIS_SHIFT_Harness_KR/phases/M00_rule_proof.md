...
- **상태**: 진행 중 — M00 구현·E2 완료 / formal Easy E1·DOD-02~07 판정은 출시 직전 playable beta로 연기
...
### 3.1 출시 직전 beta의 formal Easy 실행 게이트

- [ ] 서로 다른 신규 사용자 5명 이상이 확정됐다.
- [ ] `<HARNESS_ROOT>/docs/PLAYTEST_PROTOCOL.md`의 표본 구성 조건을 만족한다.
- [ ] 비공개 원시 기록 보관 위치와 삭제 예정일이 확정됐다.

2026-08-10 프로젝트 오너 결정으로 참가자 수·개별 기록을 확인할 지표가 없는 현재에는 formal Easy 표본을 수집하지 않고, 위 게이트를 출시 직전 playable beta까지 연기한다. 그 전에는 본 표본 테스트와 DOD-02~07 판정을 시작하지 않는다.

연기는 `PASS`, 면제 또는 임계치 완화가 아니다. 최소 표본 5명과 4/5, 30초 첫 PULSE, 90초 첫 성공, 규칙 회상 기준을 유지한다. 오너의 별도 지시에 따른 구현 체크포인트 commit·push는 가능하지만, 그 체크포인트는 M00 phase 완료나 마일스톤 DoD 통과 push가 아니다.

2026-08-14 오너는 ADR-0008에 따라 M00을 통과 처리하지 않은 채 M01의 도구 체인·모듈 경계·Hash Router·CI 체크포인트만 선행 착수하도록 승인했다. 이 예외는 M00 DOD-02~07, M02 착수 또는 릴리스 승인을 대신하지 않는다.
...
이 판정은 콘텐츠 난도 구성의 P0이며 코어 수학의 실패가 아니다. PULSE 정의, INV-005·INV-006, ADR-0002의 `Par=rank_GF2(D)`는 정상이고 변경하지 않는다. Easy `M00-MAIN-v1` 정식 이해도 테스트도 그대로 유지한다. 구조·후속 UX E2는 완료했지만 출시 직전 beta의 formal Easy 결론 전에는 DOD-02~07과 M00 완료를 보류한다.
...
`P0-DIFF-001`의 자동 구조 검증, 비게이트 사람 난도 비교와 후속 반복 UX E2는 완료됐다. DOD-02~07은 출시 직전 playable beta의 formal Easy 결론이 기록될 때까지 의도적으로 미체크 상태를 유지한다. 구현 체크포인트 push와 M00 완료를 구분하며, 연기는 사람 테스트 취소·면제나 PULSE·Par 계약 변경이 아니다.
...
| 출시 직전 beta 신규 사용자 5명+ | 동일 beta SHA·Easy 안내 → 4×4 플레이 → 종료 질문 | DOD-02~05 기준 판정 | 익명 집계표·관찰 메모 |
...
내부 파일럿 run 2회는 완료했지만 정식 본 플레이테스트는 실행하지 않았다. DOD-02~07은 체크하지 않으며, 출시 직전 playable beta에서 §3.1의 n≥5·표본 구성·비공개 기록 게이트를 연 뒤에만 동일 beta SHA의 Easy 본 표본을 집계한다. Normal·Full Rank 대조군·5×5·6×6 후보 run은 본 표본과 합산하지 않는다.
...
- 보류: 출시 직전 beta의 Easy n≥5 본 테스트 전에는 DOD-02~07과 M00을 완료하지 않는다. 구현 체크포인트 push는 이 보류 상태를 바꾸지 않는다.
...
ADR-0008의 제한형 선행 체크포인트로 M01에 다음을 전달한다. 이 인계는 M00 완료 선언이 아니다.
